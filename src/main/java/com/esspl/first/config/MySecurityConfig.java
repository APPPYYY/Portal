...
public class MySecurityConfig {
	 ...
	 * antMatchers("/register") .hasRole("ADMIN").antMatchers(HttpMethod.GET,
	 * "/ex/**").hasRole("MANAGER").anyRequest().authenticated()
	...
	protected DaoAuthenticationProvider daoAuthenticationProvider() throws Exception {
		 ...
		 * ("ADMIN").and()
		 * .withUser("ronnie").password("{noop}ronnie").roles("USER").and().withUser(
		 * "shibani") .password("{noop}shibani").roles("MANAGER");
		 */
	...
	protected SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		...
		http
				...
				.antMatchers(HttpMethod.GET, "/ex/**")
				.hasRole("MANAGER")
				.anyRequest().authenticated()
				.and().formLogin().loginPage("/signin")
				.loginProcessingUrl("/dologin").defaultSuccessUrl("/index")
				.and().logout()
